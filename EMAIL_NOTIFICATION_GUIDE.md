...
主配置文件不会展开 `${ENV_VAR}`。`resend_api_key` 必须出现在最终 YAML 中，因此应将生产配置置于版本库之外并设置为 `0600`，或由配置管理/密钥管理系统在启动前生成该文件。程序目前不会强制检查主配置文件权限。provider 的 DNS 凭据可以使用会强制检查权限的 `credentials_file`，但该字段目前不适用于 `email_notification`。
...
当前主程序没有“只发送一封测试邮件”的独立子命令。旧版本文档中的 `go run test_email.go` 入口无效，相关死代码已移除。
...
- 已发送事件历史保留约 400 天，之后在下一次状态写入时清理。
