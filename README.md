...
每个 provider 可以用 `credentials_file` 替代内联 `credentials`。文件是严格 YAML 键值映射、相对配置文件目录解析，必须是普通文件，且 group/other 不能有任何权限（例如 `0600` 或 `0400`）；不能同时配置两种来源。程序不会展开 YAML 中的 `${ENV_VAR}`。推荐独立凭据文件，因为程序会强制检查它的权限，便于按 provider 分离和轮换；使用内联凭据时，程序目前不会强制检查主配置文件模式，部署者必须自行将 `config.yaml` 设为 `0600` 或更严格。
...
已发送事件历史会保留约 400 天后清理，避免通知状态文件随证书版本长期无限增长。
